...
def get_scheculer(
    ...
    *,
    ...
    if scheduler_type == 'linear_fixed':
        return get_linear_then_fixed_scheduler(
            optimizer,
            num_warmup_steps=warmup_steps,
            last_epoch=last_epoch,
        )
...
def get_linear_then_fixed_scheduler(optimizer, num_warmup_steps, last_epoch=-1):
    lr_lambda = partial(
        _get_linear_then_fixed_scheduler_lambda,
        num_warmup_steps=num_warmup_steps,
    )
    return LambdaLR(optimizer, lr_lambda, last_epoch)

def _get_linear_then_fixed_scheduler_lambda(current_step, *, num_warmup_steps):
