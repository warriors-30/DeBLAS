...
def parse_args(args):
    ...
    args.save_dir = f"checkpoints/{args.model_config.split('/')[-1].rstrip('.json')}-optim-{args.optimizer}-lr{args.lr}-bs-{args.total_batch_size}-sq_length-{args.max_length}-totalsteps-{args.num_training_steps}-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}"
...
def main(args):
    ...
    if global_rank == 0:
        wandb.init(project="C4", name=f"{model_name}_optim-{args.optimizer}_total-bs_{args.total_batch_size}_sq-length_{args.max_length}_tr_steps_{args.num_training_steps}_lr{str(args.lr).replace('.', '_')}")
    ...
    if args.optimizer.lower() == "adam":
        ...
        optimizer = torch.optim.AdamW(trainable_params, lr=args.lr, weight_decay=args.weight_decay, betas=(args.beta1, args.beta2))
    # implement sgd
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        average_seq_length = batch["attention_mask"].sum(dim=1).float().mean()
        average_token_util = batch["attention_mask"].float().mean()
        labels = batch["input_ids"].clone()
        labels[labels == pad_idx] = -100
        ...
        if global_rank == 0:
            wandb.log({
                ...
                "gradnorm": grad_norm,
                "average_seq_length": average_seq_length,
                "average_token_util": average_token_util,
